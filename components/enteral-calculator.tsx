...
function HcpcsCodeSearch({
  ...
  // Filter HCPCS codes based on search query
  const filteredCodes = useMemo(() => {
    if (!searchQuery.trim()) return HCPCS_CODES
  ...
  // Count formulas per code
  const formulaCountByCode = useMemo(() => {
    const counts: Record<string, number> = {}
    ENTERAL_PRODUCTS.forEach((product) => {
      counts[product.hcpcsCode] = (counts[product.hcpcsCode] || 0) + 1
    })
    return counts
  }, [])

  const selectedHcpcsInfo = useMemo(
    () => HCPCS_CODES.find((c) => c.code === value),
    [value]
  )

  return (
    <div className="flex flex-col gap-2">
      <Label className="text-foreground font-semibold">HCPCS Code</Label>
              ...
              {filteredCodes.length === 0 ? (
                ...
                filteredCodes.map((hcpcs) => (
                  <button
                    key={hcpcs.code}
                    className={cn(
                      "flex flex-col gap-1 w-full px-3 py-2 text-left hover:bg-accent cursor-pointer",
                      value === hcpcs.code && "bg-accent"
                    )}
                    onClick={() => {
                      onSelect(hcpcs.code)
                      setSearchQuery("")
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="secondary" className="text-xs">
                        {formulaCountByCode[hcpcs.code] || 0} formula{(formulaCountByCode[hcpcs.code] || 0) !== 1 ? "s" : ""}
                      </Badge>
                    </div>
                    <span className="text-sm">{hcpcs.shortDescription}</span>
      ...
      {/* Show selected HCPCS info */}
      {value && selectedHcpcsInfo && (
...
function FormulaNameSearch({
  ...
  // Get products for selected HCPCS code, or all products if no code selected
  const availableProducts = useMemo(() => {
    if (hcpcsCode) {
      return ENTERAL_PRODUCTS.filter((p) => p.hcpcsCode === hcpcsCode)
    }
  ...
  const selectedProduct = useMemo(
    () => ENTERAL_PRODUCTS.find((p) => p.name === value && (hcpcsCode ? p.hcpcsCode === hcpcsCode : true)),
    [value, hcpcsCode]
  )

  return (
    <div className="flex flex-col gap-2">
      <Label className="text-foreground font-semibold">Formula Name</Label>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
          ...
          >
            {value ? (
              ...
              <span className="text-muted-foreground">
                {hcpcsCode 
                  ? `Search formulas for ${hcpcsCode}...`
                  : "Select HCPCS code first, or search all formulas..."
                }
              </span>
            )}
            <ChevronDown className="ml-2 size-4 shrink-0 opacity-50" />
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" align="start">
          <div className="flex flex-col">
            <div className="flex items-center border-b px-3">
              <Input
                placeholder="Type to search formulas..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="border-0 focus-visible:ring-0 focus-visible:ring-offset-0 h-10"
              />
            ...
            <div className="max-h-[300px] overflow-y-auto">
              {filteredProducts.length === 0 ? (
                <div className="py-6 text-center text-sm text-muted-foreground">
                  {hcpcsCode 
                    ? "No matching formula found for this HCPCS code."
                ...
                filteredProducts.map((product) => (
                  <button
                    ...
                    className={cn(
                      "flex flex-col gap-0.5 w-full px-3 py-2 text-left hover:bg-accent cursor-pointer",
                      value === product.name && "bg-accent"
                    )}
                    onClick={() => {
        ...
        <div className="rounded-lg border border-border bg-muted/40 p-3">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {selectedProduct.isPowder && <span className="ml-1 text-primary">(powder)</span>}
            </p>
          </div>
        </div>
      )}
...
interface CalculationResult {
  ...
  formulaName: string
  hcpcsCode: string
  timePeriod: TimePeriod // The user's input time period (day or month)
  inputVolume: number // The original volume amount entered by the user
}
...
function ResultsSummary({ result }: { result: CalculationResult }) {
  ...
  // For monthly input, show both the monthly total and daily average
  const inputVolumeDisplay = isPackagingUnit 
    ? `${fmt(result.inputVolume)} x ${result.volumeUnitLabel}${result.inputVolume !== 1 ? "s" : ""}`
    : `${fmt(result.inputVolume)} ${result.volumeUnitLabel}`
  
  const dailyVolumeDisplay = isPackagingUnit 
  ...
  // Build the narrative based on time period
  const buildNarrative = () => {
    if (isDirectCalorieInput) {
      if (isMonthly) {
        return `The patient receives ${fmt(result.inputVolume)} calories per month (${fmt(result.caloriesPerDay)} calories/day avg, ${densityLabel}), which equals ${fmt(unitsPerDay)} units/day. The request is for ${result.numDays} day${result.numDays !== 1 ? "s" : ""}, therefore ${fmt(result.totalUnits)} units per requested date span are required.`
      }
      return `The patient receives ${fmt(result.caloriesPerDay)} calories per day (${densityLabel}), which equals ${fmt(unitsPerDay)} units/day. The request is for ${result.numDays} day${result.numDays !== 1 ? "s" : ""}, therefore ${fmt(result.totalUnits)} units per requested date span are required.`
    }
    
    if (isMonthly) {
      return `The patient receives ${inputVolumeDisplay} per month (${dailyVolumeDisplay}/day avg), the requested ${result.formulaName} provides ${densityLabel} (${fmt(result.caloriesPerDay)} calories/day, ${fmt(unitsPerDay)} units/day), the request is for ${result.numDays} day${result.numDays !== 1 ? "s" : ""}, therefore ${fmt(result.totalUnits)} units per requested date span are required.`
    }
    
    return `The patient receives ${dailyVolumeDisplay} per day, the requested ${result.formulaName} provides ${densityLabel} (${fmt(result.caloriesPerDay)} calories/day, ${fmt(unitsPerDay)} units/day), the request is for ${result.numDays} day${result.numDays !== 1 ? "s" : ""}, therefore ${fmt(result.totalUnits)} units per requested date span are required.`
  }
      ...
      <CardContent className="flex flex-col gap-3 pt-5">
        ...
        <div className="flex flex-col gap-1.5 text-xs text-muted-foreground font-mono">
          {isDirectCalorieInput ? (
            isMonthly ? (
              <p>{`${fmt(result.inputVolume)} calories/month ÷ 30 = ${fmt(result.caloriesPerDay)} calories/day`}</p>
            ) : (
              <p>{`${fmt(result.caloriesPerDay)} calories/day (direct input)`}</p>
            )
          ) : (
            <p>{`${fmt(result.dailyVolume)} ${result.volumeUnitLabel}/day x ${fmt(kcalPerUserUnit!)} kcal/${result.volumeUnitLabel} = ${fmt(result.caloriesPerDay)} calories/day`}</p>
          )}
...
export function EnteralCalculator() {
  ...
  const [densityOverrideUnit, setDensityOverrideUnit] = useState<"kcal/mL" | "kcal/oz" | "kcal/g" | "kcal/pkg" | "kcal/carton">("kcal/mL")
  ...
  // Handlers
  const handleHcpcsSelect = useCallback((code: string) => {
    setHcpcsCode(code)
    // Clear formula when HCPCS changes (user needs to reselect formula for new code)
    setFormulaName("")
  ...
  const handleFormulaSelect = useCallback((name: string) => {
    setFormulaName(name)
    
    // If no HCPCS code selected, auto-set it from the formula
    const product = ENTERAL_PRODUCTS.find((p) => p.name === name && (hcpcsCode ? p.hcpcsCode === hcpcsCode : true))
    if (product && !hcpcsCode) {
  ...
  const handleCalculate = useCallback(() => {
    ...
    if (!isCalorieInput) {
      ...
      if (overrideValue !== null && !isNaN(overrideValue) && overrideValue > 0) {
        // Convert override value to kcal/mL or kcal/g based on the override unit selected
        let convertedOverride = overrideValue
        
        if (densityOverrideUnit === "kcal/oz") {
          // Convert kcal/oz to kcal/mL (1 oz = 29.5735 mL)
          convertedOverride = overrideValue / OZ_TO_ML
          effectiveDensityType = "kcal/mL"
        } else if (densityOverrideUnit === "kcal/pkg" || densityOverrideUnit === "kcal/carton") {
          // For package/carton, user enters total kcal per unit
          // We need to find the mL or g per that unit and calculate density
          const pkg = selectedProduct?.packaging?.[0] // Use first packaging option as reference
          if (pkg?.mlPerUnit) {
            convertedOverride = overrideValue / pkg.mlPerUnit
            effectiveDensityType = "kcal/mL"
          } else if (pkg?.gramsPerUnit) {
            convertedOverride = overrideValue / pkg.gramsPerUnit
            effectiveDensityType = "kcal/g"
          }
        } else if (densityOverrideUnit === "kcal/g") {
          effectiveDensityType = "kcal/g"
        } else {
          // kcal/mL
          effectiveDensityType = "kcal/mL"
        ...
        effectiveKcalValue = convertedOverride
      } else {
  ...
  if (volumeUnit.startsWith("pkg-") && selectedProduct?.packaging) {
    ...
    setResult({
      dailyMl,
      dailyVolume: volumeTimePeriod === "month" ? vol / 30 : vol, // Store daily volume
      ...
      inputVolume: vol, // Original input volume
    })
    setErrors([])
    ...
    <div className={cn("flex flex-col gap-6 w-full max-w-xl mx-auto", result && "calculated")}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select an HCPCS code and formula to calculate billing units.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-6">
          {/* Step 1: HCPCS Code Search */}
          <HcpcsCodeSearch
            value={hcpcsCode}
            onSelect={handleHcpcsSelect}
          />

          {/* Step 2: Formula Name Search */}
          <FormulaNameSearch
            hcpcsCode={hcpcsCode}
            value={formulaName}
                  ...
                  <div className="rounded-lg border border-border bg-muted/30 p-3 flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        min="0"
                        placeholder={
                          densityOverrideUnit === "kcal/g" ? "e.g., 4.2" :
                          densityOverrideUnit === "kcal/mL" ? "e.g., 1.0" :
                          densityOverrideUnit === "kcal/oz" ? "e.g., 30" :
                          "e.g., 250"
                        }
                      ...
                      <Select
                        ...
                        onValueChange={(val: "kcal/mL" | "kcal/oz" | "kcal/g" | "kcal/pkg" | "kcal/carton") => {
                          setDensityOverrideUnit(val)
                          setResult(null)
                        }}
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="kcal/pkg">kcal/package</SelectItem>
                          <SelectItem value="kcal/carton">kcal/carton</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    {selectedProduct && (
                      <p className="text-xs text-muted-foreground">
                        {(() => {
                          const defaultDensity = selectedProduct.isPowder && selectedProduct.kcalPerGram !== null 
                            ? `${selectedProduct.kcalPerGram} kcal/g`
                            : selectedProduct.kcalPerMl !== null 
                              ? `${selectedProduct.kcalPerMl} kcal/mL`
                              : "Not available"
                          
                          // Show packaging info for package/carton units
                          if ((densityOverrideUnit === "kcal/pkg" || densityOverrideUnit === "kcal/carton") && selectedProduct.packaging?.[0]) {
                            const pkg = selectedProduct.packaging[0]
                            const pkgKcal = pkg.kcalPerUnit ?? (pkg.mlPerUnit && selectedProduct.kcalPerMl 
                              ? Math.round(pkg.mlPerUnit * selectedProduct.kcalPerMl) 
                              : pkg.gramsPerUnit && selectedProduct.kcalPerGram 
                                ? Math.round(pkg.gramsPerUnit * selectedProduct.kcalPerGram)
                                : null)
                            return `Default: ${defaultDensity} | Reference: ${pkg.label} = ${pkgKcal ?? "N/A"} kcal`
                          }
                          
                          return `Default: ${defaultDensity}`
                        })()}
                      </p>
                    )}
                  </div>
